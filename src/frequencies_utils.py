...
class FrequencyConverter:
    ...
    def _convert(self) -> np.ndarray:
        deg_per_pixel = math.degrees(
            2 * math.atan(self.pixel_size / (2 * self.viewing_distance))
        )
        if self.mode == 0:
            ...
            raise ValueError("mode must be 0 (cpp→cpd) or 1 (cpd→cpp).")
