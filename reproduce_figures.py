...
def compute_metrics_from_rapsd(mean_rapsd_data, mean_rapsd_noise, frequencies, timesteps):
...
def plot_heatmap(x, y, z, title, xlabel, ylabel, cbar_label, output_path, cmap='viridis', smooth_sigma=1.5, force_minmax_ticks=True):
...
def main(config):
    ...
    timestep_scalar_weights, timesteps = load_training_weights(
        os.path.join(results_dir, f'training_weights.pkl')
    )
    ...
    timesteps = _to_numpy(timesteps)
    timesteps_plots = np.linspace(0.0, 1 - (1 / config.figures.plot_num_steps), config.figures.plot_num_steps)
    ...
    csf_delta_rsignal_plot = csf_delta_rsignal_numpy / np.abs(csf_delta_rsignal_numpy).max()
